...
t_monitor_if wire_if =
{
  .put_c =       wire_putc,
  .get_c =       wire_getc,
  .is_rx_ready = wire_is_rx_ready,
  .putdata =     wire_putdata,
  .getdata =     wire_getdata,
  .putdata_xmd = wire_putdata_xmd,
  .getdata_xmd = wire_getdata_xmd
};
#endif
...
{
address_Bossac=Myaddress_Bossac;
address=Myaddress;
wire=&Mywire;
}
/**
...
{
	int retval;
	retval = buffer_rx_wire[idx_rx_read];
	idx_rx_read = (idx_rx_read + 1) & (WIRE_BUFFER_SIZE - 1);
	return (retval);
}

//Send given data (polling)
uint32_t wire_putdata(void const* data, uint32_t length)
{
	uint32_t i;
	uint8_t* ptrdata;
...
uint32_t wire_getdata(void* data, uint32_t length)
{
	uint8_t* ptrdata;
	ptrdata = (uint8_t*) data;
	*ptrdata = wire_getc();
...
//*----------------------------------------------------------------------------
unsigned short wire_add_crc(char ptr, unsigned short crc)
{
...
static int putPacket(uint8_t *tmppkt, uint8_t sno)
{
	...
	for (i = 0; i < PKTLEN_128; i++)
  {
		if (size_of_data || mode_of_transfer)
    {
		...
		chksm = wire_add_crc(data, chksm);
	}

	/* An "endian independent way to extract the CRC bytes. */
	wire_putc((uint8_t) (chksm >> 8));
	wire_putc((uint8_t) chksm);

	return (wire_getc()); /* Wait for ack */
}
...
uint32_t wire_putdata_xmd(void const* data, uint32_t length)
{
	uint8_t c, sno = 1;
	uint8_t done;
...
/*----------------------------------------------------------------------------
 * \brief Used by wire_getdata_xmd to retrieve packets.
 */
static uint8_t getPacket(uint8_t *ptr_data, uint8_t sno)
{
	...
	crc = (uint16_t) wire_getc() << 8;
	crc += (uint16_t) wire_getc();
...
uint32_t wire_getdata_xmd(void* data, uint32_t length)
{
	uint32_t timeout;
	char c;
	uint8_t * ptr_data = (uint8_t *) data;
	...
	while (1)
  {
		...
		while (!(wire_is_rx_ready()) && timeout)
			timeout--;
		if (timeout)
			break;
